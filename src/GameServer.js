...
GameServer.prototype.start = function() {
    ...
    if (this.config.serverStatsPort > 0) {
		...
		this.time = this.config.serverRestart; //cowpits server restart code 
		this.time2 = this.config.serverEcho; //cowpits server restart code  
    setTimeout(function(){process.exit(1);}, this.time * 60 * 1000); //cowpits server restart code
	setTimeout(function(){echo(1);}, this.time * 60 * 1000); //cowpits server restart code
...
GameServer.prototype.resolveCollision = function(m) {
	...
	if (!check.canEat(cell) || cell.getAge() < .5) {
    return;
}
